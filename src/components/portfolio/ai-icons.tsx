import type {AiToolId} from '@/lib/portfolio-data';

/**
 * The three AI marks, matched to the ones that appear in the closing frame of
 * the transformation footage so the video → DOM hand-off reads as continuous.
 *
 * All three paint with `currentColor` and rest as off-white; brand colour is
 * applied by the caller on hover/focus only.
 *
 * Icons are decorative here — every call site supplies the accessible name.
 */

type IconProps = React.SVGProps<SVGSVGElement>;

/** Google Gemini — four-point spark. */
function GeminiMark(props: IconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden focusable="false" {...props}>
      <path d="M12 0.6c0.62 6.3 4.5 10.18 10.8 10.8v1.2c-6.3 0.62-10.18 4.5-10.8 10.8h-1.2C10.18 17.1 6.3 13.22 0 12.6v-1.2C6.3 10.78 10.18 6.9 10.8 0.6Z" />
    </svg>
  );
}

/** Anthropic / Claude — radiating burst. Built from the mark's own geometry. */
function ClaudeMark(props: IconProps) {
  const spokes = 12;
  const inner = 1.6;
  const outer = 11.2;
  return (
    <svg viewBox="0 0 24 24" aria-hidden focusable="false" {...props}>
      <g stroke="currentColor" strokeWidth={2.35} strokeLinecap="round">
...
/** OpenAI / Codex — interlocking knot. */
function CodexMark(props: IconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden focusable="false" {...props}>
...
const marks: Record<AiToolId, (props: IconProps) => React.JSX.Element> = {
  gemini: GeminiMark,
  claude: ClaudeMark,
  codex: CodexMark,
};

export function AiIcon({tool, ...props}: {tool: AiToolId} & IconProps) {
